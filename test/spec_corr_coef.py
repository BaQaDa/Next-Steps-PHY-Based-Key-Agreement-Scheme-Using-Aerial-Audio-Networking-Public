...
def spectrum_csv_from_wav(input_path, numOfRounds, start_freq, freq_num, freq_gap, lowcut, highcut, filter_order,
    ...
    for num in range(1, numOfRounds + 1):
        ...
        peaks, _ = find_peaks(x_Alice, height=0.5, distance=240)
        ax3.plot(x_Alice)
        ...
        peaks, _ = find_peaks(x_Bob, height=0.4, distance=240)
        ax4.plot(x_Bob)
        ...
        AB_BA_CorrCoef = '{0:.6f}'.format(correlationCoefficient(peaks_Alice, peaks_Bob, n))
        print(num, ' AB_BA_CorrCoef: ', AB_BA_CorrCoef)

        # Function call to correlationCoefficient. (AB_AE)
        AB_AE_CorrCoef = '{0:.6f}'.format(correlationCoefficient(peaks_Bob, peaks_Eve, n))
        print(num, 'AB_AE_CorrCoef: ', AB_AE_CorrCoef)
